...
This board is solar powered and has an ATmega1284p. The two Input Capture (ICP1, ICP3) hardware units are each connected to thre own inverting open collector transistor that pulls down the respective ICP pin when current (e.g. >7mA) is flowing through a 100 Ohm sense resistor. The captured value is accurate to within one crystal (30ppm + drift) count of the event (e.g. pulse edge that is caused by the transition of the sensor). This captured value is an excellent method for data acquisition from a flow meter or for other pulse output sensors. The board also has six digital interfaces with voltage level conversion, and four analog inputs each with current sources for current loops. The ATmega1284p can be programmed with the AVR toolchain on Debian, *Ubuntu, *Raspbian, and others (* I use these). The toolchain is also available with the Arduino IDE, and PlatformIO.
