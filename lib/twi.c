...
void twi_init(uint8_t pull_up)
{
    ...
    || defined(__AVR_ATmega644P__) || defined(__AVR_ATmega1284P__)
        // ? what does digitalWrite(SDA, 1) function call do on an m1284p
        DDRC &= ~(1 << DDC1);  // clear the ddr bit to set as an input
        PORTC |= (1 << PORTC1);  // write a one to the port bit to enable the pull-up

        // ? what does digitalWrite(SCL, 1) function call do on an m1284p
        DDRC &= ~(1 << DDC0);
        PORTC |= (1 << PORTC0); 
#else
